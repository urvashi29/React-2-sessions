...
const Products = () => {
  ...
  const [userName, setUserName] = useState("Guest1234");
  ...
  //   on intial rendering (component is mounted - display in UI)
  useEffect(() => {
    //api call
    ...
    products.map((product) => {
      return (
        <div key={product.id}>
          ...
          <button onClick={() => console.log(product.title)}>detail</button>
        </div>
      );
    })
  ) : (
  ...
  return (
    <>
      <div>{productList}</div>
      <>{x}</>
      <button onClick={() => setUserName("Admin1234")}>Update User</button>
    </>
  );
};
